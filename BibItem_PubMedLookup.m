...
+ (NSString *)referenceForPMID:(NSString *)pmid;
{
    ...
    // get the initial XML document with our search parameters in it; we're only asking for a single result
    NSString *esearch = [[[self class] baseURLString] stringByAppendingFormat:@"/esearch.fcgi?db=pubmed&retmax=1&usehistory=y&term=%@&tool=bibdesk", pmid];
    NSURL *initialURL = [NSURL URLWithString:esearch]; 
    OBPRECONDITION(initialURL);
    
    NSURLRequest *request = [NSURLRequest requestWithURL:initialURL cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:1.0];
